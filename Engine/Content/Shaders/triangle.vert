...
void main() {
    gl_Position = cameraData.viewProjection * modelData.transforms[gl_InstanceIndex] * vec4(vertexPosition, 1.0);

    fragColor           = vertexColor;
    fragTexturePosition = vertexTexturePosition;
    fragNormalPosition  = normalize((modelData.transforms[gl_InstanceIndex] * vec4(vertexNormalPosition, 0.0)).xyz);
}