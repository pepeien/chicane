...
namespace Chicane
{
    namespace FileSystem
    {
        ...
        static inline constexpr const auto SEPARATOR =
            Path::preferred_separator;

        CHICANE_CORE bool exists(const Path& inPath);
        CHICANE_CORE std::vector<Item>
        ls(const Path& inDir = ".", std::uint32_t inDepth = 0);

        CHICANE_CORE String readStringUnsigned(const Path& inFilepath);
        CHICANE_CORE std::vector<unsigned char>
        readUnsigned(const Path& inFilepath);
        ...
        CHICANE_CORE void
        write(const std::vector<char>& inData, const Path& inFilepath);
    }
}